...
if exist "VERSION.txt" (
    type VERSION.txt
    findstr /C:"0.16.0" VERSION.txt >nul 2>&1
    if errorlevel 1 (
        echo [CANH BAO] Khong phai ban 0.16.0 — tai zip moi tu GitHub Releases.
        set OK=0
    ) else (
        echo [OK] Dung phien ban 0.16.0
    )
) else (
    echo [CANH BAO] Khong co VERSION.txt — co the la ban giai nen CU hoac giai nen khong day du.
    echo            Neu app hien v0.15.x: XOA het thu muc, giai nen LAI zip 0.16.0 vao thu muc MOI.
    set OK=0
)
...
if %OK%==1 (
    ...
    echo Trong app, gooc tren trai phai hien: v0.16.0
) else (
    echo.
    ...
    echo    ImageSplatStudio-0.16.0-win-offline.zip
    echo 4. Click phai zip -^> Extract All... vao thu muc MOI
    echo 5. Chay lai KIEM_TRA.bat — phai thay VERSION.txt va v0.16.0
    echo.
    echo Link: https://github.com/HieuPC03/ConstructionModelLibrar/releases/tag/imagesplat-v0.16.0
)
echo.
pause
