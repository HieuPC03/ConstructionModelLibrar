...
def pack_preview_geometry(pts: np.ndarray, cols: np.ndarray | None) -> bytes:
    count = len(pts)
    has_colors = cols is not None and len(cols) == count
    header = struct.pack("<4sIB3x", GEOMETRY_MAGIC, count, 1 if has_colors else 0)
...
def sample_session_geometry(session_id: str, percent: float) -> tuple[np.ndarray, np.ndarray | None, dict]:
    session = get_session(session_id)
    if session is None:
        raise ValueError("Phiên preview đã hết hạn — tải lại file.")
    ...
    return sampled_pts, sampled_cols, metadata
...
def preview_from_session(session_id: str, percent: float = DEFAULT_PREVIEW_PERCENT) -> dict:
    _, _, metadata = sample_session_geometry(session_id, percent)
    return metadata
